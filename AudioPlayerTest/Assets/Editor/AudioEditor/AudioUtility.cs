	...
	public static class AudioUtility
	{
		...
		public static void PlayClip(AudioClip clip, int startSample)
		{
			...
			MethodInfo method = audioUtilClass.GetMethod(
				...
				BindingFlags.Static | BindingFlags.Public,
				null,
				new System.Type[] { typeof(AudioClip), typeof(Int32) }, null);
		...
		public static void StopClip(AudioClip clip)
		{
			...
			if (method == null)
			{
				Debug.LogError("StopAllPreviewClips method not found in AudioUtil.");
				return;
			}
		...
		{
			float previewPosition = 0;
            // Get the type of the AudioUtil class
            Type audioUtilType = Type.GetType("UnityEditor.AudioUtil, UnityEditor");

            if (audioUtilType != null)
            {
                // Get the method info for GetPreviewClipPosition
                MethodInfo methodInfo = audioUtilType.GetMethod("GetPreviewClipPosition", BindingFlags.Static | BindingFlags.Public);

                if (methodInfo != null)
                {
                    // Invoke the method passing the AudioClip and the position to get the preview position
                    // Code line that ChatGPT gave matched what I originally had and threw same error
                    // previewPosition = (float)methodInfo.Invoke(null, new object[] { clip });
					// Fixed line that ChatGPT suggested after tell it the error
                    previewPosition = (float)methodInfo.Invoke(null, null);

                    //Debug.Log("Preview Position: " + previewPosition);
                }
                else
                {
                    Debug.LogError("GetPreviewClipPosition method not found.");
                }
            }
            else
            {
                Debug.LogError("UnityEditor.AudioUtil type not found.");
            }
		...
		public static int GetCustomFilterProcessTime(MonoBehaviour behaviour)
		{
			...
			if (method == null)
			{
				throw new MissingMethodException("GetCustomFilterProcessTime method not found in AudioUtil.");
			}

			return (int)method.Invoke(null, new object[] { behaviour });
		}
		...
		public static float GetCustomFilterMaxIn(MonoBehaviour behaviour, int channel)
		{
			...
			if (method == null)
			{
				throw new MissingMethodException("GetCustomFilterMaxIn method not found in AudioUtil.");
			}

			return (float)method.Invoke(null, new object[] { behaviour, channel });
		}
		...
		public static float GetCustomFilterMaxOut(MonoBehaviour behaviour, int channel)
		{
			...
			if (method == null)
			{
				throw new MissingMethodException("GetCustomFilterMaxOut method not found in AudioUtil.");
			}
